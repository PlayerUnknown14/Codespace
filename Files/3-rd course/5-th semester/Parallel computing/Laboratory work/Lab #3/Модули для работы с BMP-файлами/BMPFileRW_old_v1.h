	...
	BITMAPINFOHEADER &bmiHeader, const char* fin)
{
	...
	fread(&(header.bfType), sizeof(unsigned short),  1, pFile);
	fread(&(header.bfSize), sizeof(unsigned long),  1, pFile);
	fread(&(header.bfReserved1), sizeof(unsigned short),  1, pFile);
	fread(&(header.bfReserved2), sizeof(unsigned short),  1, pFile);
	fread(&(header.bfOffBits), sizeof(unsigned long),  1, pFile);
...
void BMPWrite(RGBTRIPLE**& rgb, int imWidth , int imHeight, const char* fout)
{
	...
	int padding = get_row_data_padding(bmiHeader.biWidth);
	char tmp[3] = { 0,0,0 };
	// «аписываем данные изображени€ из массив структур RGBTRIPLE в файл 
	for (int i = 0; i < bmiHeader.biHeight; i++)
	{
